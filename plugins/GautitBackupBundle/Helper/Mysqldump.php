...
{
    private $fileHandler = null;
	private $zipClass =  null;
	private $file = null;
	private $fileWithoutExt = null;
    public function __construct()
    {
        if (!class_exists("\ziparchive")) {
            throw new Exception("Compression is enabled, but ziparchive lib is not installed or configured properly");
        }else{
    ...
    public function open($filename)
    {
		$this->fileWithoutExt = pathinfo($filename, PATHINFO_FILENAME);
		
		$this->file = $filename; 
        $this->fileHandler = fopen($this->fileWithoutExt, "wb");
        if (false === $this->fileHandler) {
            throw new Exception("Output file is not writable");
        }
    ...
    {       
		...
		if(fclose($this->fileHandler)){
			$this->zipClass->open($this->file,\ziparchive::CREATE);
			$this->zipClass->addFile($this->fileWithoutExt,basename($this->fileWithoutExt));
			return $this->zipClass->close();
		}
